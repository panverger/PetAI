...
// cc scanner.c -lbluetooth -lmosquitto -o scanner
...
void on_connect(struct mosquitto *mosq, void *userdata, int rc)
{
    if (rc == 0) {
	printf("Connect ok \n");    
        mosquitto_publish(mosq, NULL, "gatakia/state", 9, "connected", 0, false);
    } else {
...
int init_mqtt() {
    ...
    mosq = mosquitto_new("user132123465", true, NULL);
    ...
    if (rc != MOSQ_ERR_SUCCESS) {
        printf("Error: Unable to start the loop: %s\n", mosquitto_strerror(rc));
        mosquitto_destroy(mosq);
        mosquitto_lib_cleanup();
        return 1;
    }
...
void send_msg( int s)
{
	...
	int rc;
        ...
        sprintf(str, "%d", s); // %d is the format specifier for integers
        printf("Sending to broker RSSI %s\n",str);
	
	rc = mosquitto_publish(mosq, NULL, "gatakia/RSSI", strlen(str), str, 0, false);
...
int checkBT(){
	...
	// Set BLE scan parameters. Ορισμός παραμέτρων BluetoothLowEnergy
	...
	// Set BLE events report mask. Ορισμός φίλτρο γεγονότων
	...
	// Enable scanning. Ενεργοποίηση σαρωσης
	le_set_scan_enable_cp scan_cp;
	memset(&scan_cp, 0, sizeof(scan_cp));
	...
	// Get Results. Εμφανιση αποτελεσματων
		...
		if ( len >= HCI_EVENT_HDR_SIZE )
		{
			...
			if ( meta_event->subevent == EVT_LE_ADVERTISING_REPORT )
			{
				...
				while ( reports_count-- )
				{
					...
					if (!strcmp(addr,BTmac) ) {
					//printf("%s %s %d \n", addr," -> ", (int8_t)info->data[info->length]);
					//for (int i = 0; i < info->length; i++)
					//	printf(" %02X", (unsigned char)info->data[i]);
					
					// Αποστολή των μετρήσεων RSSI στον broker MQTT
					send_msg((int8_t)info->data[info->length]);
